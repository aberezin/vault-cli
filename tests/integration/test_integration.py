...
def test_integration_cli(cli_runner, clean_vault):
    ...
    assert call(cli_runner, ["get", "a", "--key", "value"]).output == "b\n"

    assert (
        call(cli_runner, ["get", "a", "--key", "value", "--yaml"]).output
        == "--- b\n...\n"
    )

    call(cli_runner, ["set", "b", "--stdin"], input="key1=val1 key2=val2")

    assert call(cli_runner, ["get", "b"]).output == "---\nkey1: val1\nkey2: val2\n"
    ...
    call(cli_runner, ["set", "c/d", "value=e"])

    assert call(cli_runner, ["get", "c/d", "--key", "value"]).output == "e\n"
